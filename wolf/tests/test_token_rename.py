...
# https://github.com/Carrene/wolf/wiki/User-Stories#rename-token
class RenameTokenTestCase(WebTestCase):
    url = '/apiv1/tokens'
    ...
    def test_rename_token(self):
        mockup_token_id = self.mockup_token_id

        result, ___ = self.request(
            As.provider, 'RENAME', f'{self.url}/{mockup_token_id}',
            params=[
                FormParameter('name', 'new-name')
            ]
        )
