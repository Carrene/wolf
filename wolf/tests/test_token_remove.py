...
# https://github.com/Carrene/wolf/wiki/User-Stories#remove-token
class RemoveTokenTestCase(WebTestCase):
    url = '/apiv1/tokens'
    ...
    def test_remove_token(self):
        ...
        result, ___ = self.request(As.provider, 'REMOVE', f'{self.url}/{mockup_token_id}')

        self.assertIn('id', result)

        # Really was removed?
        self.request(As.provider, 'REMOVE', f'{self.url}/{mockup_token_id}', expected_status=404)
        self.request(As.provider, 'GET', f'{self.url}/{mockup_token_id}', expected_status=404)
