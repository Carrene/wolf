...
class TestVerifyPrimitive(LocalApplicationTestCase):
    ...
    def mockup(cls):
        ...
        cls.active_token3 = active_token3 = Token()
        active_token3.name = 'name3'
        active_token3.phone = 3
        active_token3.bank_id = 2
        active_token3.expire_date = datetime.now() + timedelta(minutes=1)
        active_token3.seed = \
            b'y\x19*\x9aT\xa2*\xd53\xf0\x80\xda\xd7\xdd\x9d\xc6E\xa7\xa5\x94'
        active_token2.is_active = True

        active_token3.cryptomodule = mockup_cryptomodule_length_4
        session.add(active_token3)
        ...
        cls.pinblock3 = EncryptedISOPinBlock(active_token3.id)
        ...
        cls.valid_otp_token3_time = cls.pinblock3.encode('7110').decode()
        ...
        cls.invalid_otp_token3_time = cls.pinblock3.encode('123456').decode()
    ...
    def test_verify_bad_primitive(self):
        with TimeMonkeyPatch(self.valid_time), self.given(
                'Verifying time based OPT with bad primitive query',
                \
    ...
    def test_verify_primitive_no(self):
        with TimeMonkeyPatch(self.valid_time), self.given(
                ...
                \
                    f'/apiv1/tokens/token_id: {self.active_token3.id}/codes'
                    f'/code: {self.valid_otp_token3_time}',
                'VERIFY',
            ...
            when(
                'Trying to verify an invalid code',
                url_parameters=given | dict(
                    code=self.invalid_otp_token3_time,
                )
            )
            assert status == '604 Invalid code'
            ...
            session = self.create_session()
            token = session.query(Token) \
                .filter(Token.id == self.active_token3.id) \
                .one()
            token.expire_date = datetime(1970, 3, 1)
            session.commit()
            when('When token is expired')
